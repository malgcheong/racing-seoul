...
- **키보드**: `↑↓←→` / `WASD` 운전 · `F` 상향등(앞차 양보 요구) · `C` 1인칭↔3인칭 전환
- **게임패드** (표준 매핑): 좌스틱/십자 조향 · `RT` 가속 · `LT` 브레이크 · `X` 상향등 · `Y` 시점 전환 — 트리거·스틱은 아날로그
- **터치**: 터치 기기에서 가상 조이스틱 자동 표시 — 좌측 스틱(X=조향 아날로그 · 위=가속 · 아래=브레이크) + 우측 가속/브레이크 버튼 · 상향등 · 시점
...
src/
  ...
  game/
    ...
    minimap.js · physics.js · sky.js · sounds.js · npr.js · carPreview.js
  map/
